...
checkapps() {
	for a in ${required_packages[@]}; do
		[[ ! -e $(command -v $a) ]] && { echo "Scripti calistirmadan once \"$a\" programini yukleyin..."; exit 1; }
	done
}
...
#~ check config
checkconfig() {
	if [[ ! -e /etc/alarm_du_v2.conf ]]; then
		echo "/etc/alarm_du_v2.conf dosyasi github'tan cekiliyor..."
		curl -fsSL https://raw.githubusercontent.com/monobilisim/mono.sh/main/.bin/alarm_du_v2.conf > /etc/alarm_du_v2.conf
		echo "/etc/alarm_du_v2.conf dosyasini duzenleyin ve scripti tekrar calistirin"
		exit 0
	else
		. /etc/alarm_du_v2.conf
	fi
}
...
checkvariable() {
    local arr=($@)
    for a in ${arr[@]}; do
        [[ ! -n ${!a} ]] && { echo "\"${a^}\" degiskeni /etc/alarm_du_v2.conf dosyasi icinde tanimli degil"; exit 1; }
    done
}
...
convertToBytes() {
	...
	case "$unit" in
		"KiB")
			printf "%.f" "$(echo "${value%"KiB"} * 1024" | bc)"
			;;
		"MiB")
			printf "%.f" "$(echo "${value%"MiB"} * 1024^2" | bc)"
			;;
		"GiB")
			printf "%.f" "$(echo "${value%"GiB"} * 1024^3" | bc)"
			;;
		"TiB")
			printf "%.f" "$(echo "${value%"TiB"} * 1024^4" | bc)"
			;;
		"KB"|"K")
			printf "%.f" "$(echo "${value%"KB"} * 1000" | bc)"
			;;
		"MB"|"M")
			printf "%.f" "$(echo "${value%"MB"} * 1000^2" | bc)"
			;;
		"GB"|"G")
			printf "%.f" "$(echo "${value%"GB"} * 1000^3" | bc)"
			;;
		"TB"|"T")
			printf "%.f" "$(echo "${value%"TB"} * 1000^4" | bc)"
			;;
		*)
			printf "%.f" "${value//%/}"
			;;
...
getPartitionInformations() {
	...
	for a in ${diskout[@]}; do
		...
		if [[ -n $(echo $a | awk '{print $2}' | grep -E 'ext4|ext3|ext2|xfs|nfs4') ]]; then
			local usage=$(df -h --output="used" $mount | sed '1d; s/%//g' | tr -d ' ')
			local total=$(df -h --output="size" $mount | sed '1d; s/%//g' | tr -d ' ')
			local percentage=$(df -h --output="pcent" $mount | sed '1d; s/%//g' | tr -d ' ')
		elif [[ $(echo $a | awk '{print $2}') == "btrfs" ]]; then
			local usage=$(btrfs fi us / 2>/dev/null | grep "^    Used:" | awk '{print $2}')
			local total=$(btrfs fi us / 2>/dev/null | grep "Device size:" | awk '{print $3}')
			local percentage=$(convertToBytes $(echo "scale=2; $(convertToBytes $usage) / $(convertToBytes $total) * 100" | bc))
		elif [[ $(echo $a | awk '{print $2}') == "zfs" ]]; then
			local usage=$(zfs list -H -o used $partition)
			local total=$(zpool list -H -o size $partition)
			local percentage=$(convertToBytes $(echo "scale=2; $(convertToBytes $usage) / $(convertToBytes $total) * 100" | bc))
		fi
...
checkAndReportPartitions() {
	...
	local message="{\"text\": \"[ÇÖZÜLDÜ] - [ $(hostname) ] Bölüm kullanım seviyesi aşağıdaki bölümler için $threshold% seviyesinin altina indi;\n\`\`\`\n"
	...
	for a in $(echo $json | jq -r ".[] | select(.percentage | tonumber < $threshold) | [.percentage, .usage, .total, .partition, .mount] | @tsv"); do
		...
		[[ -f "/tmp/alarm_du/${mount//\//_}" ]] && { table+="\n$(printf '%-5s | %-10s | %-10s | %-35s | %-35s' $percentage% $usage $total $partition ${mount//sys_root/})"; underthreshold=1; rm -f /tmp/alarm_du/${mount//\//_}; }
	done
	message+="$table\n\`\`\`\"}"
	IFS=$OLDIFS
	...
	if [[ -n $jsondata ]]; then
		local message="{\"text\": \"[UYARI] - [ $(hostname) ] Bölüm kullanım seviyesi aşağıdaki bölümler için $threshold% sevıyesinin üstüne çıktı;\n\`\`\`\n"
		local table="$(printf '%-5s | %-10s | %-10s | %-35s | %s' '%' 'Used' 'Total' 'Partition' 'Mount Point')"
		table+='\n'
		for z in $(seq 1 95); do table+="$(printf '-')"; done
		OLDIFS=$IFS
		...
		for b in ${jsondata[@]}; do
			...
			[[ "$mount" == "/" ]] && mount="/sys_root"
			[[ -f "/tmp/alarm_du/${mount//\//_}" ]] && { [[ "$(cat /tmp/alarm_du/${mount//\//_})" == "$(date +%Y-%m-%d)" ]] && { overthreshold=0; continue; }; } || { echo "$(date +%Y-%m-%d)" > /tmp/alarm_du/${mount//\//_}; overthreshold=1; }
